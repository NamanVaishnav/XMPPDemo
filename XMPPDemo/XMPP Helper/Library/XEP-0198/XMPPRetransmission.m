...
- (void)xmppStreamDidAuthenticate:(XMPPStream *)sender
{
    ...
    [self.storage enumerateMonitoredElementsWithBlock:^(NSUUID *elementId, XMPPElement *element, NSDate *timestamp) {
        dispatch_async(self.moduleQueue, ^{
            if ([element isKindOfClass:[XMPPPresence self]] && [element wasDelayed]) {
                NSArray *array = @[elementId];
                [self.storage clearMonitoredElementsWithIds:array];
                [self.retransmittedElements removeObjectForKey:elementId];
            } else {
