...
- (XMPPMessageArchiving_Message_CoreDataObject *)notificationMessageWith:(NSString *)messageJid
                                                    managedObjectContext:(NSManagedObjectContext *)moc
{
    XMPPMessageArchiving_Message_CoreDataObject *result = nil;
    ...
    NSString *predicateFrmt = @"elementID == %@";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFrmt,messageJid];
...
-(void)setStatus:(NSString *) status ForMessageWithElementId:(NSString *) elementID {
    ...
    [self scheduleBlock:^{
        ...
        NSString *predicateFrmt = @"elementID == %@";
        NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFrmt,elementID];
        ...
        if (results == nil || error) {
            ...
            NSDate *timestamp = [result.message delayedDeliveryDate];
            if (timestamp) {
                if ([status isEqualToString:@"READ"]) {
                    result.readTimestamp = timestamp;
                } else if ([status isEqualToString:@"DELIVERED"]) {
                    result.deliverTimestamp = timestamp;
                }
            } else {
                if ([status isEqualToString:@"READ"]) {
                    result.readTimestamp = [[NSDate alloc] init];
                } else if ([status isEqualToString:@"DELIVERED"]) {
                    result.deliverTimestamp = [[NSDate alloc] init];
                }
            }
        }
    }];
...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    if (![self messageContainsRelevantContent:message])
    {
        ...
        if (!isComposing)
        {
            ...
            else
            {
                ...
                //                    [self setStatus:@"DELIVERED" ForMessageWithElementId:message.receiptResponseID];
                //                }
                //
                //                if (message.hasReadReceiptResponse) {
                //                    [self setStatus:@"READ" ForMessageWithElementId:message.readReceiptResponseID];
                //                }
    ...
    [self scheduleBlock:^{
        ...
        else
        {
            ...
            {
                archivedMessage = [self notificationMessageWith:[message elementID] managedObjectContext:moc];
            ...
            NSString *status = archivedMessage.status;
            if (isOutgoing) {
                if ([message elementForName:@"archived"]) {
                    archivedMessage.status = @"CLOUD";
                } else {
                    archivedMessage.status = @"PENDING";
                }
            } else {
                if ([status isEqualToString:@"READ"]) {
                    
                } else {
                    archivedMessage.status = @"DELIVERED";
                }
            }
            ...
            if (didCreateNewArchivedMessage) // [archivedMessage isInserted] doesn't seem to work
            {
                ...
                NSLog(@"Message: %@ from XMPP stored in archive", messageBody);
            }
            else
            {
